...
@section('title', 'Edit Daily Teaching Work')
        ...
        <div class="col-md-10">
            <div class="card">
                <div class="card-header">
                    <h4>Edit Daily Teaching Work</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.daily-teaching-work.update', $dailyTeachingWork) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control @error('date') is-invalid @enderror" 
                                           id="date" name="date" value="{{ old('date', $dailyTeachingWork->date->format('Y-m-d')) }}" required>
                                    @error('date')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('class_name') is-invalid @enderror" 
                                        ...
                                        @foreach($classes as $class)
                                            <option value="{{ $class }}" {{ old('class_name', $dailyTeachingWork->class_name) == $class ? 'selected' : '' }}>
                                                {{ $class }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('section') is-invalid @enderror" 
                                        ...
                                        @foreach($sections as $section)
                                            <option value="{{ $section }}" {{ old('section', $dailyTeachingWork->section) == $section ? 'selected' : '' }}>
                                                {{ $section }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('subject') is-invalid @enderror" 
                                        ...
                                        @foreach($subjects as $subject)
                                            <option value="{{ $subject }}" {{ old('subject', $dailyTeachingWork->subject) == $subject ? 'selected' : '' }}>
                                                {{ $subject }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('teacher_id') is-invalid @enderror" 
                                        ...
                                        @foreach($teachers as $teacher)
                                            <option value="{{ $teacher->id }}" {{ old('teacher_id', $dailyTeachingWork->teacher_id) == $teacher->id ? 'selected' : '' }}>
                                                {{ $teacher->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('period_number') is-invalid @enderror" 
                                           id="period_number" name="period_number" value="{{ old('period_number', $dailyTeachingWork->period_number) }}" min="1" max="10">
                                    @error('period_number')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('topic_covered') is-invalid @enderror" 
                                           id="topic_covered" name="topic_covered" value="{{ old('topic_covered', $dailyTeachingWork->topic_covered) }}" maxlength="255">
                                    @error('topic_covered')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('academic_session') is-invalid @enderror" 
                                           id="academic_session" name="academic_session" value="{{ old('academic_session', $dailyTeachingWork->academic_session) }}" placeholder="e.g. 2024-2025">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('teaching_summary') is-invalid @enderror" 
                                      id="teaching_summary" name="teaching_summary" rows="4">{{ old('teaching_summary', $dailyTeachingWork->teaching_summary) }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-text">Select multiple files to add to existing attachments (PDF, DOC, DOCX, JPG, PNG, MP4)</div>
                            @error('attachments')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                                    ...
                                    <div class="card">
                                        <div class="card-body text-center">
                                            @if(in_array($attachment['extension'], ['pdf']))
                                                <i class="fas fa-file-pdf text-danger" style="font-size: 1.5em;"></i>
                                            @elseif(in_array($attachment['extension'], ['doc', 'docx']))
                                                <i class="fas fa-file-word text-primary" style="font-size: 1.5em;"></i>
                                            @elseif(in_array($attachment['extension'], ['jpg', 'jpeg', 'png']))
                                                <i class="fas fa-file-image text-info" style="font-size: 1.5em;"></i>
                                            @elseif(in_array($attachment['extension'], ['mp4', 'avi', 'mov']))
                                                <i class="fas fa-file-video text-warning" style="font-size: 1.5em;"></i>
                                            @else
                                                <i class="fas fa-file text-secondary" style="font-size: 1.5em;"></i>
                                            @endif
                                            <div class="mt-1">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('homework_description') is-invalid @enderror" 
                                      id="homework_description" name="homework_description" rows="2">{{ old('homework_description', $dailyTeachingWork->homework['description'] ?? '') }}</textarea>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control @error('homework_due_date') is-invalid @enderror" 
                                       id="homework_due_date" name="homework_due_date" value="{{ old('homework_due_date', $dailyTeachingWork->homework['due_date'] ?? '') }}">
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            ...
                            <button type="submit" class="btn btn-primary">Update Daily Teaching Work</button>
                        </div>
                    </form>
                </div>
            </div>
