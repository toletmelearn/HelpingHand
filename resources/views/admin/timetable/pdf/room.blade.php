...
<head>
    <meta charset="utf-8">
    <title>Room Timetable - {{ $room }}</title>
    <style>
        body {
        ...
        .class-section { color: #555; display: block; font-size: 9px; }
        .teacher { color: #777; display: block; font-size: 8px; font-style: italic; }
    </style>
</head>
<body>
    <div class="header">
        <h1>{{ config('app.name', 'School') }}</h1>
        <h2>Room Timetable — {{ $room }}</h2>
                    ...
                    @foreach($days as $day)
                        ...
                        <td class="{{ $isNonTeaching ? 'non-teaching' : '' }}">
                            ...
                            @elseif($slot)
                                ...
                                <span class="teacher">{{ $slot->teacher->short_name ?? $slot->teacher->name ?? '' }}</span>
                            @endif
                        </td>
                    @endforeach
                </tr>
