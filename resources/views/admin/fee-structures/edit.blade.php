        ...
        <div class="col-12">
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 text-white">Update Fee Structure</h5>
                    <a href="{{ route('admin.fee-structures.index') }}" class="btn btn-light btn-sm text-primary">
                        <i class="fas fa-arrow-left me-1"></i> Back
                    </a>
                </div>
                <div class="card-body">
                    <form id="feeStructureForm" action="{{ route('admin.fee-structures.update', $feeStructure->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('academic_year') is-invalid @enderror"
                                        ...
                                        @foreach($academicSessions as $session)
                                            <option value="{{ $session->code }}" {{ old('academic_year', $feeStructure->academic_year) == $session->code ? 'selected' : '' }}>
                                                {{ $session->code }}
                                            </option>
                                        @endforeach
                                        @if(!$academicSessions->pluck('code')->contains($feeStructure->academic_year))
                                            <option value="{{ $feeStructure->academic_year }}" selected>{{ $feeStructure->academic_year }} (legacy value)</option>
                                        @endif
                                    </select>
                                    @else
                        ...
                        <!-- Overall Frequency input (kept for compatibility) -->
                        <input type="hidden" name="frequency" value="{{ $feeStructure->frequency ?? 'custom' }}">

                        <hr class="my-4">
                        
                        <h5 class="mb-3"><i class="fas fa-list me-1 text-primary"></i> Edit Fee Heads & Amounts</h5>
                            ...
                            <table class="table table-hover align-middle" id="feeTypesTable">
                                ...
                                <tbody>
                                    @foreach($feeTypes as $type)
                                        @php
                                            $item = $feeStructure->feeStructureItems->firstWhere('fee_type_id', $type->id);
                                            $hasItem = !empty($item);
                                        @endphp
                                        <tr class="fee-head-row" data-id="{{ $type->id }}">
                                            <td class="text-center">
                                                <input type="checkbox" class="form-check-input fee-checkbox" id="chk_{{ $type->id }}" {{ $hasItem ? 'checked' : '' }}>
                                                <input type="hidden" name="fee_heads[{{ $type->id }}][fee_type_id]" value="{{ $type->id }}" {{ $hasItem ? '' : 'disabled' }} class="row-input">
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="input-group">
                                                    ...
                                                    <input type="number" class="form-control fee-amount row-input" 
                                                           name="fee_heads[{{ $type->id }}][amount]" 
                                                           value="{{ $hasItem ? $item->amount : '' }}"
                                                           step="0.01" min="0" {{ $hasItem ? '' : 'disabled' }} required placeholder="Enter amount">
                                                </div>
                                            </td>
                                            <td class="text-end">
                                                <button type="button" class="btn btn-outline-primary btn-sm customize-btn" {{ $hasItem ? '' : 'disabled' }}>
                                                    <i class="fas fa-sliders-h me-1"></i> Customize
                                                </button>
                                            </td>
                                        </tr>
                                        <!-- Collapsible customization block -->
                                        <tr class="customization-row d-none" id="custom_row_{{ $type->id }}" data-pre-months="{{ $hasItem && is_array($item->charge_months) ? implode(',', $item->charge_months) : ($hasItem && is_string($item->charge_months) ? implode(',', json_decode($item->charge_months, true) ?? []) : '') }}">
                                                ...
                                                <div class="card card-body mb-0 shadow-none border bg-white">
                                                    ...
                                                    <div class="row g-3">
                                                        <div class="col-md-3">
                                                            ...
                                                            <select class="form-select billing-frequency-select row-input" name="fee_heads[{{ $type->id }}][billing_frequency]" {{ $hasItem ? '' : 'disabled' }}>
                                                                ...
                                                                <option value="one_time" {{ $hasItem && ($item->billing_frequency ?? '') == 'one_time' ? 'selected' : '' }}>One-Time (Charged Once, Entire Enrollment)</option>
                                                        ...
                                                        <div class="col-md-2">
                                                            ...
                                                            <input type="number" class="form-control row-input" name="fee_heads[{{ $type->id }}][due_day]" value="{{ $hasItem ? $item->due_day : '' }}" min="1" max="31" {{ $hasItem ? '' : 'disabled' }} placeholder="e.g. 10">
                                                        </div>
                                                        <div class="col-md-3">
                                                            <label class="form-label small fw-semibold text-muted">Late Fee Rule</label>
                                                            <select class="form-select row-input" name="fee_heads[{{ $type->id }}][late_fee_rule_id]" {{ $hasItem ? '' : 'disabled' }}>
                                                                <option value="">No Late Fee</option>
                                                                @foreach($lateFeeRules as $rule)
                                                                    <option value="{{ $rule->id }}" {{ $hasItem && $item->late_fee_rule_id == $rule->id ? 'selected' : '' }}>{{ $rule->name }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="col-md-4 installment-config-col d-none">
                                                            ...
                                                            <button type="button" class="btn btn-outline-info btn-sm d-block w-100 configure-installments-btn">
                                                                <i class="fas fa-edit me-1"></i> Edit Installment Amounts
                                                            </button>
                                                        </div>
                                                        <div class="col-md-4 exam-config-col d-none">
                                                            <label class="form-label small fw-semibold text-muted">Select Exam</label>
                                                            <select class="form-select row-input exam-select" name="fee_heads[{{ $type->id }}][exam_id]" {{ $hasItem ? '' : 'disabled' }}>
                                                                <option value="">Select Exam Event</option>
                                                                <option value="1" {{ $hasItem && $item->exam_id == 1 ? 'selected' : '' }}>Unit Test 1</option>
                                                                <option value="2" {{ $hasItem && $item->exam_id == 2 ? 'selected' : '' }}>Half Yearly</option>
                                                                <option value="3" {{ $hasItem && $item->exam_id == 3 ? 'selected' : '' }}>Annual Exam</option>
                                                            </select>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <input type="hidden" class="charge-months-input row-input" name="fee_heads[{{ $type->id }}][charge_months_raw]" value="" {{ $hasItem ? '' : 'disabled' }}>
                                                    <input type="hidden" class="installments-raw-input row-input" name="fee_heads[{{ $type->id }}][installments_raw]" value="{{ $hasItem ? $item->installments->map(fn($inst) => $inst->month.':'.$inst->amount)->join(';') : '' }}" {{ $hasItem ? '' : 'disabled' }}>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            ...
                            <button type="submit" class="btn btn-primary" id="saveBtn">
                                <i class="fas fa-save me-1"></i> Update Structure
                            </button>
                        </div>
                    </form>
                </div>
...
$(document).ready(function() {
    ...
    function renderMonthPills($row, selectedMonths = null) {
        ...
        if (frequency === 'monthly' || frequency === 'custom') {
            ...
            pillsHtml = `<button type="button" class="btn btn-sm btn-primary month-pill active" data-value="OneTime" disabled>Charged Once</button>`;
            activeMonths = ['OneTime'];
    ...
    // Initialize already checked rows on page load
    $('.fee-checkbox:checked').each(function() {
        const $row = $(this).closest('.fee-head-row');
        const typeId = $row.data('id');
        const $customRow = $(`#custom_row_${typeId}`);
        const preMonths = $customRow.data('pre-months');
        const months = preMonths ? preMonths.split(',') : null;
    ...
    $(document).on('change', '.billing-frequency-select', function() {
        ...
        $customRow.find('.charge-months-input').val('');
        renderMonthPills($customRow);
    });
    ...
    $(document).on('click', '.month-pill', function() {
        ...
        if (freq === 'yearly' || freq === 'session_wise_admission' || freq === 'one_time' || freq === 'session_wise_continuing' || freq === 'exam_wise') return;
    ...
    function updateTotals() {
        ...
        $('.fee-checkbox:checked').each(function() {
            ...
            if (freq === 'monthly') {
                ...
                yearlyTotal += amount * count;
            } else if (freq === 'yearly' || freq === 'session_wise_admission' || freq === 'one_time' || freq === 'session_wise_continuing') {
                yearlyTotal += amount;
            } else if (freq === 'custom') {
                yearlyTotal += amount;
            } else if (freq === 'exam_wise') {
                yearlyTotal += amount * 3;
            }
        });
    ...
    $('#feeStructureForm').submit(function(e) {
        ...
        $('#saveBtn').prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-1"></i> Saving Changes...');
    });
});
</script>
@endsection